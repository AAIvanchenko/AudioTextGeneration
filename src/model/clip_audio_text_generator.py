...
from .audio_clip_inference import AudioCLIPInference
...
class AudioCLIPTextGenerator(torch.nn.Module):
    # Based on https://github.com/YoadTew/zero-shot-image-to-text/blob/main/model/ZeroCLIP.py
    def __init__(
            self,
            seed=0,
            lm_model='gpt-2',
        ...
        # Зададим зерно для генерации
        torch.manual_seed(seed)
        np.random.seed(seed)

        # Инициализируем LM модель
        self.context_prefix = ''
        if lm_model == 'gpt-neo':
            self.lm_tokenizer = AutoTokenizer.from_pretrained('EleutherAI/gpt-neo-125M')
            self.lm_model = AutoModelForCausalLM.from_pretrained('EleutherAI/gpt-neo-125M', output_hidden_states=True)
        elif lm_model == 'gpt-2':
            self.lm_tokenizer = AutoTokenizer.from_pretrained('gpt2-medium')
            self.lm_model = AutoModelForCausalLM.from_pretrained('gpt2-medium', output_hidden_states=True)
            self.context_prefix = self.lm_tokenizer.bos_token
        self.lm_model.to(self.device)
        self.lm_model.eval()

        # Заморозим веса LM модели
        for param in self.lm_model.parameters():
            param.requires_grad = False

        # Зададим запрещённые токены
        ...
        # Инициализируем AudioCLIP
        self.clip = AudioCLIPInference(clip_checkpoints)
    ...
    def get_audio_feature(self, audio: torch.Tensor):
        with torch.no_grad():
            audio_features = self.clip.encode_audio(audio)
            audio_features = audio_features / audio_features.norm(dim=-1, keepdim=True)

            return audio_features.detach()

    def get_txt_features(self, text: list[str]):
    ...
    def clip_loss(self, probs, context_tokens):
        ...
        with torch.no_grad():
            similiraties = (self.audio_features @ text_features.T).reshape(probs.size(0), -1)
